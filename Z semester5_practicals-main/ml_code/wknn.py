...
if ordering == "assending":
    ...
    print(f"Predicted label : {np.argmax([np.sum(1./(y[np.argsort(dist)][:k]+1e-12) * ((y[np.argsort(dist)])[:k] == 0)), np.sum(1./(y[np.argsort(dist)][:k]+1e-12) * (y[np.argsort(dist)])[:k] == 1)])}")
else:
    print(f"Top {k} neighbors    : {(dist[np.argsort(dist)])[-k:][::-1]}")
    print(f"Top {k} class label  : {(y[np.argsort(dist)])[-k:][::-1]}")
    print(f"Predicted label : {np.argmax([np.sum(y[np.argsort(dist)][-k:][::-1] * (y[np.argsort(dist)])[-k:][::-1] == 0), np.sum(y[np.argsort(dist)][-k:][::-1] * (y[np.argsort(dist)])[-k:][::-1] == 1)])}")
